...
class PhotoCollection: NSObject, ObservableObject {
    ...
    // PhotoAssets propriedade que permite usar a coleção de materiais de foto da mesma forma como vc usa um vetor. Ela é publicada de PhotoCollection, um objeto observável. Vc pode responder a mudanças nos recursos de foto, como quando fotos são adicionadas ou apagadas.
    //let asset = photoCollection.photoAssets[4] //usando índice
    //let count = photoCollection.photoAssets.count //contar todas as fotos da sua coleção
    ...
    private func refreshPhotoAssets(_ fetchResult: PHFetchResult<PHAsset>? = nil) async {
        ...
        func photoLibraryDidChange(_ changeInstance: PHChange) {
            guard let changes = changeInstance.changeDetails(for: photoAssets.fetchResult) else { return }
            // A chamada para refreshPhotoAssets já é `async` e estará no Main Actor.
            Task {
                await refreshPhotoAssets(changes.fetchResultAfterChanges)
            }
        }
    }
